...
const longAddress = computed(() => {
  const region = selectedAddress.value.region
  return region.province + region.city + region.region +selectedAddress.value.detail
})
...
const checkOrderList = async () => {
  // 购物车结算
  if(mode.value === 'cart'){
    const { data } = await checkOrderAPI(mode.value, {
...
const submitOrder = async () => {
  if(selectedAddress.value.address_id === undefined){
    return showToast('请选择配送地址！')
  }
  if(mode.value === 'cart'){
    await submitOrderAPI(mode.value,{
      cartIds:cartIds.value,
      remark:remark.value
    })
  }
